...
void regenerate_framebuffers(renderer_backend *backend, vulkan_swapchain *swapchain,
...
b8 vulkan_renderer_backend_initialize(renderer_backend *backend, const char *application_name,
  ...
  context.framebuffer_height = (cached_framebuffer_height != 0) ? cached_framebuffer_width : 600;
...
// Debugger
  ...
  // Swapchain
  vulkan_swapchain_create(&context, context.framebuffer_width, context.framebuffer_height,
                          &context.swapchain);

  vulkan_renderpass_create(&context, &context.main_renderpass, 0, 0, context.framebuffer_width,
                           context.framebuffer_height, 0.f, 0.f, .2f, 1.f, 1.f, 0);
  ...
  for (u8 i = 0; i < context.swapchain.max_frames_in_flight; ++i) {
    ...
    // "rendered". This will prevent the application from waiting indefinitly for the first frame to
    // render since it cannot be rendered until a frame is "rendered" before it.
    vulkan_fence_create(&context, TRUE, &context.in_flight_fences[i]);
  }

  // In flight fences should not yet exist at this point, so clear the list. These are stored in
...
void vulkan_renderer_backend_shutdown(renderer_backend *backend) {
  ...
  // Renderpass
  vulkan_renderpas_destroy(&context, &context.main_renderpass);
